...
class AccountBankStatementsImportFile(models.TransientModel):
    ...
    def _get_default_file_format_id(self):
        return self.env["account.journal"].browse(
            self.env.context.get("journal_id")).default_file_format_id
    ...
    import_file = fields.Binary(
        required=True,
        help="Wrap the file imported from your bank",
    )
    ...
    def create_statement(self, transactions_list):
        statement_vals = {
            'name':
            'Statement Of ' + str(datetime.today().date()).replace('-', '/'),
            'journal_id':
    ...
    def import_file_button(self):
        self.ensure_one()
        logger.info("Start to import bank statement file %s",
                    self.import_filename)
        logger.info("File format %s", self.file_format_id.parser_model_name)

        transactions_list = self.env[
            self.file_format_id.parser_model_name].parse(
                self.import_file, self.import_filename)
        ...
        if self.env.context.get("return_regular_interface_action"):
            action = (
                self.env.ref("account.action_bank_statement_tree").sudo().read(
                    [])[0])
            action.update({
                "view_mode": "form,tree",
                "views": False,
